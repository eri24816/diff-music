...
class SymbolicRepresentation:
    ...
    def __init__(self, token: Tensorable|None=None, token_type: Tensorable|None=None, pos: Tensorable|None=None, device: torch.device=torch.device("cpu")):
        '''
        ...
        if token is None or token_type is None or pos is None:
            assert token is None and token_type is None and pos is None, 'if one argument is provided, all arguments must be provided'
    ...
    def __add__(self, other: 'SymbolicRepresentation'):
        return SymbolicRepresentation(cat_to_right(self.token, other.token, dim=1), cat_to_right(self.token_type, other.token_type, dim=1), cat_to_right(self.pos, other.pos, dim=1))
    ...
    def add_frame(self):
        self.pos = cat_to_right(self.pos, self.pos[-1]+1, dim=1)
